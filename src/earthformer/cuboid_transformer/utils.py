...
class SpatioTemporalWaveletTransform(nn.Module):
    def __init__(self, wavelet='haar'):
        super(SpatioTemporalWaveletTransform, self).__init__()
        self.wavelet = wavelet

    def forward(self, x):
        """
        x: 输入数据，形状为 (batch_size, channels, height, width, time_steps)
        这里假设输入为五维张量，前四维是空间数据，最后一维是时间序列
        """
        # 对空间维度进行小波变换
        ...
        # 对每一个时间步的每个通道和批次进行小波变换
        for b in range(batch_size):
            for c in range(channels):
                for t in range(time_steps):
                    # 获取当前时刻的空间数据 (height, width)
                    spatial_data = x[b, c, :, :, t].cpu().numpy()  # 转换为NumPy数组

                    # 进行2D小波变换
                    coeffs2 = pywt.dwt2(spatial_data, self.wavelet)
                    LL, (LH, HL, HH) = coeffs2  # 低频和高频子带

                    # 将结果转换回PyTorch张量，并存储在对应位置
                    transformed[b, c, :, :, t] = torch.tensor(np.stack([LL, LH, HL, HH]), dtype=torch.float32).to(
                        x.device)
...
# # 生成随机输入数据
# input_data = torch.randn(batch_size, channels, height, width, time_steps)

# # 创建模型并前向传播
# model = SpatioTemporalModel(wavelet='haar')
